...
export default function Admin() {
  ...
  // --- Auth (uses same password as VITE_ADMIN_PASS) ---
  ...
  // --- Nested update helper (creates missing objects/arrays) ---
  ...
  // --- Save to Server (Netlify Function using ADMIN_SAVE_KEY) ---
  ...
  async function saveToServer() {
    try {
      ...
      const res = await fetch('/.netlify/functions/cms', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Admin-Key': pass }, // must match ADMIN_SAVE_KEY in Netlify
        body: text,
      });
      if (!res.ok) throw new Error(await res.text());
      alert('Saved for everyone. Open your site on your phone to confirm.');
    } catch (e) {
  ...
  // Explicit “save to this browser” (local preview)
  ...
  return (
    <div>
      ...
      <div className="card">
        ...
        <p className="muted" style={{ marginTop: 8 }}>Server save uses your login password to authorize.</p>
      </div>
            ...
            return (
              <div key={k} className="card">
                ...
                <input
                  ...
                  value={p.description}
                  onChange={(e) =>
                    update('seo.pages', { ...data.seo.pages, [k]: { ...p, description: e.target.value } })
                  }
                />
                <input
              ...
              <div key={k} className="card">
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      update('theme.pages', { ...data.theme.pages, [k]: { ...t, color: e.target.value } })
                    }
                  />
                </label>
                <label>
                  ...
                  <input
                    value={t.size}
                    onChange={(e) =>
                      update('theme.pages', { ...data.theme.pages, [k]: { ...t, size: e.target.value } })
                    }
                  />
                </label>
                <label>
                  ...
                  <input
                    value={t.repeat}
                    onChange={(e) =>
                      update('theme.pages', { ...data.theme.pages, [k]: { ...t, repeat: e.target.value } })
                    }
                  />
                </label>
                <label>
                  ...
                  <input
                    value={t.position}
                    onChange={(e) =>
                      update('theme.pages', { ...data.theme.pages, [k]: { ...t, position: e.target.value } })
                    }
                  />
                </label>
                <label>
                  ...
                  <input
                    value={t.attachment}
                    onChange={(e) =>
                      update('theme.pages', { ...data.theme.pages, [k]: { ...t, attachment: e.target.value } })
                    }
                  />
                </label>
                <ImagePicker
        ...
        <div className="grid cols-3" style={{ marginTop: 8 }}>
          {data.home.missionTeasers.map((t, i) => (
            <div key={i} className="card">
              ...
              <button
                onClick={() =>
                  update('home.missionTeasers', data.home.missionTeasers.filter((_, x) => x !== i))
                }
              >
                Remove
              </button>
      ...
      <Section title="Services">
        <div className="grid cols-2">
          {data.services.map((s, i) => (
            <div key={s.id || i} className="card">
              ...
              <button onClick={() => update('services', data.services.filter((_, x) => x !== i))}>
                Remove
              </button>
            </div>
          ))}
        ...
        <button
          onClick={() =>
            update('services', [...data.services, { id: 'new', title: 'New', description: '' }])
          }
        >
          Add Service
        </button>
      ...
      <Section title="Training">
        <div className="grid cols-2">
          {data.training.map((t, i) => (
            <div key={t.id || i} className="card">
              ...
              <button onClick={() => update('training', data.training.filter((_, x) => x !== i))}>
                Remove
              </button>
            </div>
          ))}
        ...
        <button
          onClick={() =>
            update('training', [...data.training, { id: 'new', title: 'New', description: '' }])
          }
        >
          Add Training
        </button>
      ...
      <Section title="About – Mission, Gallery, Team, FAQs">
        ...
        <div className="grid cols-2">
          {data.about.team.map((m, i) => (
            <div key={i} className="card">
              ...
              />
              <button onClick={() => update('about.team', data.about.team.filter((_, x) => x !== i))}>
                Remove
              </button>
            </div>
          ))}
        </div>
        <button onClick={() => update('about.team', [...data.about.team, { name: '', role: '', photo: '', bio: '' }])}>
          Add Team Member
        </button>
        ...
        {data.about.faqs.map((f, i) => (
          <div key={i} className="card">
            ...
            />
            <button onClick={() => update('about.faqs', data.about.faqs.filter((_, x) => x !== i))}>
              Remove
            </button>
          </div>
        ))}
